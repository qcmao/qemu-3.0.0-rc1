...
static void dump_search(TranslationBlock *tb)
{
    target_ulong data[TARGET_INSN_START_WORDS] = { tb->pc };
    uintptr_t host_pc = (uintptr_t)tb->tc.ptr;
    uint8_t *p = tb->tc.ptr + tb->tc.size;
    int i, j, m,num_insns = tb->icount;
    qemu_log("Index      hostpc              targetpc           extradata...\n");
    for (i = 0; i < num_insns; ++i) {
        for (j = 0; j < TARGET_INSN_START_WORDS; ++j) {
            data[j] += decode_sleb128(&p);
        }
        host_pc += decode_sleb128(&p);
        qemu_log("[%08d] 0x%016" PRIxPTR ":",i,host_pc );
        for (m = 0; m < TARGET_INSN_START_WORDS; m++) {
            qemu_log(" 0x%016" PRIx64, (uint64_t)data[m]);
        }
        qemu_log("\n");
    }
}
...
TranslationBlock *tb_translate_code_static(target_ulong pc,
...
buffer_overflow:
    ...
    tb->cs_base = cs_base;
    tb->flags = flags;
    tb->cflags = cflags|CF_STATIC_TRANS; /* static translate code */
    tb->trace_vcpu_dstate = *cpu->trace_dstate;
    tcg_ctx->tb_cflags = cflags;
    ...
    if(1){
        qemu_log("\n\n ====================\n");
        qemu_log("|==\n");
        qemu_log("|==      Begin TB=%p,pc=[%"PRIx64",%"PRIx64")\n",tb,
                           (uint64_t)tb->pc, (uint64_t)(tb->pc+tb->size));
        qemu_log("|==\n");
        qemu_log(" ====================\n");
        qemu_log("TargetCode:\n");
        target_disas_static(qemu_logfile, cpu, tb->pc, tb->size);
        qemu_log("\n=====================\n");
        qemu_log("TCG ops:\n");
...
void tb_static_trans_loop(target_ulong pc,
                              target_ulong cs_base,
                              uint32_t flags, int cflags)
{
    TranslationBlock *tb;
    target_ulong pc_next;
    void *prologue, *epilogue, *end;
    prologue=tcg_ctx->code_gen_prologue;
    epilogue=tcg_ctx->code_gen_epilogue;
    end=tcg_ctx->code_gen_ptr;
    qemu_log("\nPrologue:\n");
    log_disas(prologue,(unsigned char*)epilogue-(unsigned char*)prologue);
    qemu_log("\nEpilogue:\n");
    log_disas(epilogue,(unsigned char*)end-(unsigned char*)epilogue);
    pc_next = pc;
    qemu_set_log(CPU_LOG_TB_OUT_ASM | CPU_LOG_TB_OP_IND | CPU_LOG_TB_OP_OPT);
    do{
        tb = tb_translate_code_static(pc_next, cs_base, flags, cflags);
        if(tb)
            pc_next = tb->pc + tb->size;
    }while(tb && tb->size);
}
