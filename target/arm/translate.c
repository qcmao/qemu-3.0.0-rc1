...
static bool insn_crosses_page(CPUARMState *env, DisasContext *s)
{
     ...
     */
    uint16_t insn;
    if(s->base.tb->cflags&CF_STATIC_TRANS){
        uint16_t insn16;
        if(qemu_lduw_static_code(s->pc, &insn16)){ //fail
            return true;
        }
        insn = insn16;
    }else {
...
static void arm_tr_translate_insn(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    if(dcbase->tb->cflags&CF_STATIC_TRANS){
        if(qemu_ldl_static_code(dc->pc, &insn)){ //fail
            gen_exception(EXCP_PREFETCH_ABORT, syn_swstep(dc->ss_same_el, 0, 0),
                      default_exception_el(dc));
...
static void thumb_tr_translate_insn(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    if(dcbase->tb->cflags&CF_STATIC_TRANS){
        uint16_t insn16;
        if(qemu_lduw_static_code(dc->pc, &insn16)){ //fail
            gen_exception(EXCP_PREFETCH_ABORT, syn_swstep(dc->ss_same_el, 0, 0),
                      default_exception_el(dc));
            dcbase->is_jmp = DISAS_NORETURN;
            return;
    ...
    if (!is_16bit) {
        ...
        if(dcbase->tb->cflags&CF_STATIC_TRANS){
            ...
            insn2 = arm_lduw_code(env, dc->pc, dc->sctlr_b);
        }
