...
static int
target_read_memory_static (bfd_vma memaddr,
                    bfd_byte *myaddr,
                    int length,
                    struct disassemble_info *info)
{
    //CPUDebug *s = container_of(info, CPUDebug, info);
    qemu_static_code_read(memaddr, myaddr, length);
    return 0;
}
...
void target_disas_static(FILE *out, CPUState *cpu, target_ulong code,
                  target_ulong size)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);
    target_ulong pc;
    ...
    s.cpu = cpu;
    s.info.read_memory_func = target_read_memory_static;
    s.info.buffer_vma = code;
    s.info.buffer_length = size;
    s.info.print_address_func = generic_print_address;
    s.info.cap_arch = -1;
